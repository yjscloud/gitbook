...
在root目录下创建zabbix\_install目录，目录下包含zabbix\_package.tar.gz文件，该文件包含Zabbix安装的所需要的所有rpm包，安装脚本也在该目录下。
...
打开浏览器，访问[http://x.x.x.x/zabbix](http://x.x.x.x/zabbix) \(x.x.x.x为服务器IP\)
